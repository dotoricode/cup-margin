  ...
  <main className="cm-shell min-h-screen overflow-x-hidden text-[#061b31]">
   <section className="relative cm-gradient-hero">
    <div className="relative mx-auto w-full max-w-7xl px-4 py-5 sm:px-8 lg:px-10">
     <nav className="cm-nav rounded-xl px-3 py-3 sm:px-4">
      ...
      <button
       ...
       className="cm-button-secondary inline-flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-sm font-bold md:hidden"
   ...
   <section id="calculator" className="pb-32 sm:pb-8">
    <div className="mx-auto mb-3 w-full max-w-7xl px-5 sm:px-8 lg:px-10">
      <div className="rounded-2xl border border-[#c7d3e3] bg-white px-4 py-3 shadow-[rgba(11,37,69,0.08)_0px_12px_28px_-18px]">
       <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
         <p className="text-xs font-bold text-[#0b2545]">언제 쓰나요</p>
         <h1 className="mt-1 text-2xl font-semibold text-[#061b31]">아메리카노 가격만 바꿔도 한 잔에 얼마 남는지 바로 봅니다.</h1>
    ...
    <div className="mx-auto grid w-full max-w-7xl gap-4 px-5 sm:px-8 lg:grid-cols-[minmax(0,1fr)_420px] lg:px-10">
     <div className="order-1 rounded-2xl border border-[#e5edf5] bg-white p-5 shadow-[rgba(50,50,93,0.2)_0px_38px_60px_-42px,rgba(0,0,0,0.1)_0px_18px_36px_-24px] sm:p-6 lg:order-1">
      <div className="flex flex-col gap-4 border-b border-[#e5edf5] pb-5 sm:flex-row sm:items-start sm:justify-between">
       <div>
        ...
        <h2 className="mt-3 text-3xl font-bold text-[#061b31] sm:text-4xl">한 메뉴부터 빠르게 계산하기</h2>
      ...
      {isCalculatorOpen ? (
       <div>
        {showFirstVisitHint ? (
         <div className="mt-5 rounded-xl border border-[#d8e3ee] bg-[#f8fafc] px-4 py-3 text-sm font-semibold leading-6 text-[#0b2545]">
       ...
       <p className="mb-3 text-xs font-bold text-[#0b2545]">1단계 · 매달 빠지는 돈</p>
       <label className="block">
       <span className="block text-sm font-bold text-[#273951]">
        월 고정 운영비
       </span>
       <input
        ...
        className="mt-3 w-full bg-transparent text-3xl font-semibold text-[#061b31] outline-none placeholder:text-[var(--cm-muted)]"
       />
       ...
       {input.menus.map((menu, index) => {
        ...
        <article id={`menu-input-${menu.id}`} key={menu.id} className="scroll-mt-6 rounded-2xl border border-[#e5edf5] bg-white p-4 shadow-[rgba(23,23,23,0.05)_0px_14px_30px_-20px]">
         <div className="flex flex-col gap-3 border-b border-[#edf2f7] pb-4 sm:flex-row sm:items-center sm:justify-between">
          <label className="flex-1">
           ...
           <input
            ...
            className="mt-1 w-full bg-transparent text-2xl font-semibold text-[#061b31] outline-none placeholder:text-[var(--cm-muted)]"
           />
          </label>
          <button
           type="button"
         ...
         <div className="mt-4 space-y-4">
          <div>
           <p className="mb-2 text-xs font-bold text-[#0b2545]">3단계 · 가격과 월 판매량</p>
          ...
          <div>
           <p className="mb-2 text-xs font-bold text-[#0b2545]">4단계 · 기본 원가</p>
           <div className="grid gap-3 sm:grid-cols-2">
           ...
           <details className="mt-3 rounded-2xl bg-[#f8fbff] p-3 ring-1 ring-[#e5edf5]">
            <summary className="cursor-pointer text-sm font-bold text-[#0b2545]">
         ...
         {menuResult && menuHealth ? (
          <div className="mt-4 grid gap-2 rounded-xl bg-[#f8fbff] p-3 ring-1 ring-[#d8e3ee] sm:grid-cols-3">
           <div>
            <p className="text-[11px] font-semibold text-[var(--cm-muted)]">한 잔 이익</p>
            <p className="mt-1 text-lg font-bold text-[#061b31]">{formatWon(menuResult.profitPerCup)}</p>
           </div>
           <div>
            <p className="text-[11px] font-semibold text-[var(--cm-muted)]">월 이익</p>
            <p className="mt-1 text-lg font-bold text-[#061b31]">{formatWon(menuResult.monthlyProfit)}</p>
           </div>
           <div className="flex items-center sm:justify-end">
            <span className={`rounded-md px-3 py-1.5 text-xs font-bold ${menuHealth.className}`}>{menuHealth.label}</span>
           </div>
          </div>
         ) : null}
      ...
      {input.menus.length > 1 ? (
       <button type="button" onClick={addMenu} className="mt-5 w-full rounded-xl border border-dashed border-[#9fb3cc] bg-[#f3f7fb] px-4 py-4 text-sm font-bold text-[#0b2545] transition hover:bg-[#e8f0f8]">
        + 메뉴 추가하기
       </button>
      ) : null}
       </div>
       ...
       >
        <span className="block text-sm font-bold text-[#0b2545]">내 카페 값으로 바꿔볼 수 있어요</span>
        <span className="mt-1 block text-sm leading-6 text-[var(--cm-muted)]">월 운영비와 메뉴별 판매량을 입력하면 오른쪽 결과가 바로 바뀝니다.</span>
       </button>
      )}
     </div>

     <div className="order-2 space-y-4 lg:order-2 lg:self-start">
      ...
      <details
       ...
       className="rounded-2xl border border-[#e5edf5] bg-white p-4 shadow-[rgba(11,37,69,0.08)_0px_16px_36px_-28px]"
      >
       <summary className="cursor-pointer text-sm font-bold text-[#0b2545]">
 ...
 return (
  <label className={`${field.compact ? "" : ""} rounded-xl border border-[#d8e3ee] bg-white p-4 transition focus-within:border-[#0b2545] focus-within:shadow-[rgba(11,37,69,0.16)_0px_14px_26px_-18px]`}>
   ...
   <input
    ...
    inputMode="numeric"
    placeholder="0"
    className="mt-3 w-full bg-transparent text-xl font-semibold text-[#061b31] outline-none placeholder:text-[var(--cm-muted)]"
   />
...
function ResultPanel({
 ...
 return (
  <div className="rounded-2xl border border-[#e5edf5] bg-white p-6 shadow-[rgba(50,50,93,0.18)_0px_30px_45px_-34px,rgba(0,0,0,0.08)_0px_18px_36px_-24px]">
   <p className="text-sm font-bold text-[#0b2545]">가게 전체 결과</p>
   <h2 className="mt-2 text-3xl font-semibold text-[#061b31]">이번 달 예상 손익</h2>
   {selectedMenu && decision ? (
    <div className="mt-5 rounded-2xl border border-[#c7d3e3] bg-[#f8fbff] p-4">
     <p className="text-xs font-bold text-[var(--cm-muted)]">선택 메뉴 한 줄 결론</p>
     <div className="mt-2 flex flex-wrap items-center gap-2">
      <span className="rounded-md bg-[#0b2545] px-3 py-1.5 text-xs font-bold text-white">{decision.label}</span>
      <p className="text-2xl font-semibold text-[#061b31]">{decision.title}</p>
     </div>
     ...
     <details className="mt-4 rounded-xl bg-white px-4 py-3 text-sm font-bold leading-6 text-[#0b2545] ring-1 ring-[#d8e3ee]">
      <summary className="cursor-pointer text-sm font-bold text-[#0b2545]">근거 수식 보기</summary>
      <p className="mt-3">판매가 - 변동비 - 잔당 고정비 배분 = 한 잔 이익</p>
      <p className="mt-1 text-lg text-[#061b31]">{selectedFormula}</p>
   ...
   {hasCalculatorInput ? (
    ...
    <div className="mt-5 rounded-xl border border-[#c7d3e3] bg-[#f8fbff] px-4 py-4 text-[#273951]">
     <p className="font-bold">아직 계산 전이에요</p>
...
function Metric({ label, value, emphasis = false, dark = false }: { label: ReactNode; value: string; emphasis?: boolean; dark?: boolean }) {
 return (
  <div className={`rounded-xl p-4 ${dark ? "bg-white/10" : emphasis ? "bg-[#f0f0ff]" : "bg-[#f8fbff]"}`}>
   ...
   <p className={`mt-2 text-xl font-semibold ${dark ? "text-white" : "text-[#061b31]"}`}>{value}</p>
  </div>
 );
}
