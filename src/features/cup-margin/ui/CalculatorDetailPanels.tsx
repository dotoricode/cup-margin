...
export function PriceRiskPanel({
 ...
 return (
  <div id="price-risk-panel" className="scroll-mt-4 rounded-2xl border border-[#c7d3e3] bg-white p-6 shadow-[rgba(11,37,69,0.18)_0px_28px_56px_-34px] transition-all duration-500 ease-out">
   <div className="flex items-start justify-between gap-3">
    <div>
     <p className="text-sm font-bold text-[#0b2545]">가격 바꿔보기</p>
     <h3 className="mt-2 text-[22px] font-semibold leading-tight text-[#061b31] sm:text-2xl">몇 잔까지 줄어도 괜찮을까요?</h3>
    </div>
   ...
   <div className="mt-4 grid gap-3 sm:grid-cols-2">
    <label className="rounded-2xl border border-[#d8e3ee] bg-white p-4 transition focus-within:border-[#0b2545]">
     ...
     <input
      ...
      type="text"
      inputMode="numeric"
      className="mt-3 w-full bg-transparent text-xl font-semibold text-[#061b31] outline-none"
     />
   ...
   <div className="mt-3 flex flex-wrap gap-2">
    {scenarioButtons.map((rate) => (
     <button
      ...
      className={`rounded-md px-3 py-1.5 text-xs font-bold transition hover:-translate-y-0.5 ${volumeChangeRate === rate ? "bg-[#0b2545] text-white shadow-[rgba(11,37,69,0.28)_0px_12px_22px_-14px]" : "bg-white text-[#0b2545] ring-1 ring-[#c7d3e3]"}`}
     >
      {rate > 0 ? `+${rate}%` : `${rate}%`}
     </button>
    ))}
...
export function MenuBreakdownPanel({
 ...
 return (
  <div className="rounded-2xl border border-[#e5edf5] bg-white p-4 shadow-[rgba(23,23,23,0.06)_0px_18px_44px_-28px] sm:p-6">
   <h3 className="text-2xl font-semibold text-[#061b31]">메뉴별 손익</h3>
   {hasCalculatorInput ? (
    <div className="mt-4 rounded-2xl border border-[#c7d3e3] bg-[#f8fbff] p-4">
     <p className="text-sm font-bold text-[#0b2545]">돈 새는 메뉴 TOP 3</p>
    ...
    {hasCalculatorInput ? menus.map((menu) => {
     ...
     return (
      <div key={menu.id} className="cm-card-muted rounded-2xl p-4">
       ...
       <p className="mt-3 text-2xl font-semibold text-[#061b31]">{formatWon(menu.profitPerCup)} / 잔</p>
       ...
       <button
        ...
        className="mt-3 w-full rounded-xl border border-[#c7d3e3] bg-white px-3 py-2 text-sm font-bold text-[#0b2545] transition hover:-translate-y-0.5 hover:bg-[#f3f7fb]"
       >
        이 메뉴 500원 올리면?
       </button>
      </div>
