    ...
    <main className="cm-shell min-h-screen text-[#061b31]">
      <section className="relative overflow-hidden">
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
          <nav className="cm-nav mt-5 rounded-2xl px-3 py-3 sm:px-4">
          ...
          <div id="top" className="grid items-center gap-12 pb-20 pt-12 lg:grid-cols-[1fr_0.92fr] lg:pb-28 lg:pt-24">
            <div className="max-w-3xl">
              ...
              <p className="mt-7 max-w-full text-lg font-normal leading-8 text-[var(--cm-muted)] sm:max-w-2xl sm:text-xl">
                <span className="block">판매가·원가·월 판매량·월세를 넣으면</span>
                <span className="block">한 잔 순이익과 가격 인상 참고선을 바로 계산합니다.</span>
              </p>
              <p className="mt-3 max-w-2xl rounded-2xl border border-[#d8e3ee] bg-white/80 px-4 py-3 text-sm font-bold leading-6 text-[#0b2545]">
          ...
          <div className="mt-10 grid gap-4 lg:grid-cols-3">
            {pricingPlans.map((plan) => (
              <div key={plan.name} className={`rounded-2xl border p-6 ${plan.highlighted ? "border-[#9fb3cc] bg-white text-[#061b31]" : "border-white/10 bg-white/[0.07]"}`}>
                <p className={`text-sm font-bold ${plan.highlighted ? "text-[#0b2545]" : "text-white/70"}`}>{plan.name}</p>
                ...
                <p className={`mt-2 text-sm ${plan.highlighted ? "text-[var(--cm-muted)]" : "text-white/70"}`}>{plan.caption}</p>
...
function QuickStartCard() {
  return (
    <div className="cm-card rounded-[28px] p-5 sm:p-6">
      ...
      <h2 className="mt-3 text-3xl font-medium leading-tight tracking-[-0.04em] text-[#061b31]">내 메뉴 하나부터 바로 계산하세요.</h2>
      <div className="mt-6 grid gap-3">
        {[
          ...
          <div key={label} className="flex items-center justify-between rounded-2xl bg-[#f5f8fb] px-4 py-3">
            <span className="text-sm font-bold text-[var(--cm-muted)]">{label}</span>
    ...
    <section id="demo" className="mx-auto w-full max-w-7xl px-5 py-16 sm:px-8 lg:px-10">
      <div className="grid gap-5 rounded-[2rem] border border-[#dbe5f0] bg-white p-6 shadow-[rgba(15,23,42,0.10)_0px_26px_60px_-34px] sm:p-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div>
          ...
          <p className="mt-4 text-base leading-7 text-[var(--cm-muted)]">샘플 결과를 먼저 보고, 바로 내 매장 값으로 바꿔 계산할 수 있습니다.</p>
...
function SectionEyebrow({ children }: { children: React.ReactNode }) {
  return <p className="text-sm font-black uppercase tracking-[0.12em] text-[#0b2545]">{children}</p>;
}
